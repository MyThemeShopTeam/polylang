...
class PLL_RankMath {
	...
	public function init() {
		...
		if ( PLL() instanceof PLL_Frontend ) {
			add_filter( 'option_rank-math-options-titles', array( $this, 'rank_math_translate_titles' ) );
	 ...
	 * @return array
	 */
	protected function _rank_math_translate_titles( $options, $titles ) {
	 ...
	 * @return array
	 */
	public function rank_math_translate_titles( $options ) {
		if ( PLL() instanceof PLL_Frontend ) {
			foreach ( get_post_types( array( 'public' => true, '_builtin' => false ) ) as $t ) {
				if ( pll_is_translated_post_type( $t ) ) {
					$options = $this->_rank_math_translate_titles( $options, array( 'pt_' . $t . '_title', 'pt_' . $t . '_description' ) );
				}
			}
			foreach ( get_post_types( array( 'has_archive' => true, '_builtin' => false ) ) as $t ) {
				if ( pll_is_translated_post_type( $t ) ) {
					$options = $this->_rank_math_translate_titles( $options, array( 'pt_' . $t . '_archive_title', 'pt_' . $t . '_archive_description' ) );
				}
			}
			foreach ( get_taxonomies( array( 'public' => true, '_builtin' => false ) ) as $t ) {
				if ( pll_is_translated_taxonomy( $t ) ) {
					$options = $this->_rank_math_translate_titles( $options, array( 'tax_' . $t . '_title', 'tax_' . $t . '_description' ) );
				}
			}
		}
		return $options;
	 ...
	 * @return array
	 */
	public function rank_math_home_url_white_list( $arr ) {
